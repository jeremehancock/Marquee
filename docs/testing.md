...
4. **Both directions, from every category.** All → Movies → Shows → Seasons →
   Collections and back.
5. **The two ends.** Drag right on All, and left on Collections. Each should move
...
6. **Commit, abandon, and change your mind.** Past a third of the screen and
...
7. **A flick.** A short, fast drag should commit even though it never travelled a
   third of the screen.
8. **With a search active.** Type a search, then swipe. The destination must open
   filtered by the same search — not unfiltered, and not showing the previous
   category's matches.
9. **Straight after a change.** Delete a poster, or run an import, then swipe.
   The destination must reflect what just happened. If it shows the poster you
   deleted, a stale held copy was trusted.
10. **With a tray open.** Open the actions tray, then try to drag sideways over
...
11. **Interrupt it.** Start a drag and lock the phone, or switch apps, or rotate
...
12. **Back and forward.** Swipe through several categories, then use the browser's
    back gesture. It must walk back through them.
13. **Deep scroll.** Scroll several pages into a category, swipe away, swipe back.
...
14. **Reduced motion.** Turn it on at the system level. The grid must **still
...
15. **On a desktop with a mouse.** Nothing should have changed: no drag, and tab
