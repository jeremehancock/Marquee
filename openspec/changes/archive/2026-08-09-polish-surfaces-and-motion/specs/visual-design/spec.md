...
The application's visual language: the design token contract every surface draws
from, how surfaces are layered and separated from one another, how elements
respond to pointer and focus, and the app-wide guarantee that motion can be
turned off.

This capability owns *how things look and how they move*. It owns no screen and
no control — the other capabilities keep owning what their screens do. When a
requirement here and a requirement elsewhere both touch the same element, the
other capability governs the behaviour and this one governs its appearance.
