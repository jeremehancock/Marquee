...
The system SHALL NOT offer the Plex server address on the settings screen, and
SHALL NOT offer the auto-import settings there.

The server address is not merely a setting: it is an assertion only someone with
host access can make, and it is what stops the first stranger who reaches an
...
The auto-import settings are withheld for a different reason: the schedule is
still fixed into the container at boot, so a control for it would appear to work
and would not. A setting SHALL NOT be offered before the behavior it names can
follow it.

#### Scenario: The server address is not editable here
- **WHEN** a user opens the settings screen
- **THEN** no field offers the Plex server address

#### Scenario: Auto-import is not offered yet
- **WHEN** a user opens the settings screen
- **THEN** no control claims to enable, disable, or schedule the background
  import
